...
export async function accessSettings(value:unknown,ownerId:string){
  ...
  for(const member of data.members){
    ...
    const users=query.includes('@')
      ? await all<{id:string;email:string}>("SELECT id,email FROM users WHERE status='active' AND LOWER(email)=LOWER(?) LIMIT 2",query)
      : await all<{id:string;email:string}>("SELECT id,email FROM users WHERE status='active' AND LOWER(display_name)=LOWER(?) LIMIT 2",query);

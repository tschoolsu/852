...
import { Upload, X, ChevronDown, ChevronUp, Trash2, Share2 } from 'lucide-react';
import { UploadAccessDialog, privateAccess, type UploadAccess } from './upload-access-dialog';
...
export function UploadQueue({csrf,parent,destination,disabled,setBusy,onDone,owner}:{owner:{name:string;email:string};csrf:string;parent:string;destination:string;disabled:boolean;setBusy:(value:boolean)=>void;onDone:()=>Promise<void>}){
  ...
  const openAccess=async(id:string)=>{
    if(lock.current||disabled)return;setAccessError('');
  ...
  const saveAccess=async(access:UploadAccess)=>{
    if(lock.current)return;lock.current=true;setPending(true);setBusy(true);setAccessError('');let changed=false;const errors:string[]=[];
    try{for(const entry of entries.filter(e=>(accessTarget==='all'||e.id===accessTarget)&&e.status!=='unknown')){
      if(entry.resourceId){try{const response=await fetch(`/api/resources/${entry.resourceId}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({operation:'access-config',csrf,access}),signal:AbortSignal.timeout(60000)});const data=await response.json() as {error?:string;url?:string};if(!response.ok)throw new Error(data.error||'權限更新失敗');update(entry.id,{access:structuredClone(access),shareUrl:data.url,error:''});changed=true;}catch(error){errors.push(`${entry.title||entry.file.name}：${(error as Error).message}`);}}
      else update(entry.id,{access:structuredClone(access),error:''});
    }if(changed)await onDone();if(errors.length)setAccessError(errors.join('\n'));else setAccessTarget('');
    ...
    try{
      for(const entry of ready){
        ...
        try{
          ...
          const data=await response.json() as {error?:string;id?:string;url?:string};
          ...
          update(entry.id,{status:'done',resourceId:data.id,shareUrl:data.url});changed=true;
  ...
  return <>
    <input ref={picker} id="multi-upload-picker" aria-label="選擇多個上傳檔案" type="file" multiple hidden disabled={disabled} onChange={event=>{
      ...
      setEntries(current=>[...current,...files.map(file=>({id:crypto.randomUUID(),file,title:file.name,description:'',parent,destination,status:'ready' as const,error:'',access:privateAccess()}))]);setCollapsed(false);
    }}/>

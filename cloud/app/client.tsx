...
function DetailDialog({detail,selected,close,error,status,mode,setMode,busy,mutate,folders,setFolders,setMessage,openDelete}:{detail:Detail|null;selected:Item|null;close:()=>void;error:string;status:string;mode:string;setMode:(v:''|'edit'|'move'|'share'|'versions')=>void;busy:boolean;mutate:(op:string,data?:Record<string,unknown>|FormData)=>Promise<void>;folders:Item[];setFolders:(v:Item[])=>void;generatedLink:string;setMessage:(v:string)=>void;openDelete:()=>void}){
  ...
  if(!selected.permission)return <Dialog open onOpenChange={open=>!open&&close()}><DialogContent><DialogHeader><DialogTitle>你目前沒有此項目的存取權</DialogTitle><DialogDescription>{selected.title}</DialogDescription></DialogHeader><p>{formatKind(selected)} · {selected.owner_name} · {formatDate(selected.created_at)}</p><p>{selected.description||'沒有說明'}</p>{selected.kind==='file'&&<p>檔案大小：{formatSize(selected.size_bytes)||'未知'}</p>}<DialogFooter><Button onClick={close}>完成</Button></DialogFooter></DialogContent></Dialog>;

...
export default function AuthPanel({onSignedIn}:{onSignedIn:()=>Promise<void>}) {
  ...
  const title={login:'登入',register:'註冊帳號',forgot:'忘記密碼',complete:'設定密碼'}[mode];
  return <main className="login-page"><section className="login-intro"><div className="brand light"><span className="brand-mark">T</span><strong>學生會內部服務</strong></div><div><h1>學生會檔案<br/>管理系統</h1><p>使用你的學校信箱與獨立密碼登入</p></div><small>T-Files © 2026 TSchool 學生會數位部</small></section><section className="login-panel"><div className="login-card"><p className="eyebrow">學生會檔案庫</p><h2>{title}</h2>
    {mode!=='login'&&<p>{mode==='register'?'先驗證學校信箱，再自行設定顯示名稱與密碼。':mode==='forgot'?'輸入學校信箱，我們會寄送密碼設定連結。':'請設定獨立密碼。新帳號需填寫顯示名稱；既有帳號留白則保留原名稱。'}</p>}
    ...
    <form onSubmit={submit}><fieldset disabled={pending} className="form-stack">
      ...
      <Button type="submit" disabled={pending}>{pending?'處理中…':mode==='login'?'登入':mode==='complete'?'儲存新密碼':'寄送驗證信'}</Button>
    </fieldset></form>
    <div className="auth-links">{mode==='login'?<><button disabled={pending} onClick={()=>switchMode('register')}>註冊帳號</button><button disabled={pending} onClick={()=>switchMode('forgot')}>忘記密碼</button></>:<button disabled={pending} onClick={()=>switchMode('login')}>返回登入</button>}</div>
